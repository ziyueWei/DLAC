...
from train_modules.evaluation_metrics import evaluate, evaluate_wos_layer
import torch
import tqdm
import random
import numpy as np
    ...
    def run(self, data_loader, epoch, mode='TRAIN', label_desc_loader=None):
        ...
        for batch in tqdm.tqdm(data_loader):
            if mode != "TRAIN":
                ...
                logits, text_repre, label_repre_positive, label_repre_negative, tail_logits = self.model([batch, mode, label_repre])
            ...
            if mode == 'TRAIN':
                ...
                loss_tail = self.criterion(tail_logits, batch['tail_label'].to(self.config.train.device_setting.device),
                                                               recursive_constrained_params)
        ...
        if mode == 'TRAIN':
            ...
            metrics1 = evaluate_wos_layer(predict_probs,
                               target_labels,
                               self.vocab,
                               self.config.eval.threshold)
            logger.info("Performance at epoch %d --- Precision: %f, Recall: %f, Micro-F1: %f, Macro-F1: %f,"
                        "l1_micro_f1: %f, l2_micro_f1: %f, l1_macro_f1: %f, l2_micro_f1: %f,"
                        % (epoch, metrics1['precision'], metrics1['recall'], metrics1['micro_f1'], metrics1['macro_f1'],
                           metrics1['l1_micro_f1'], metrics1['l2_micro_f1'], metrics1['l1_macro_f1'], metrics1['l2_macro_f1']))
            return metrics
