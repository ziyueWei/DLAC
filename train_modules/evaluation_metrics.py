...
def evaluate(epoch_predicts, epoch_labels, vocab, threshold=0.5, top_k=None):
    ...
    for sample_predict, sample_gold in zip(epoch_predicts, epoch_gold):
        ...
        for i in range(len(confusion_count_list)):
            for predict_id in sample_predict_id_list:
                confusion_count_list[i][predict_id] = confusion_count_list[i][predict_id] + 1

        # count for the gold and right items
        for gold in sample_gold:
            gold_count_list[gold] = gold_count_list[gold] + 1
            for label in sample_predict_id_list:
                if gold == label:
                    right_count_list[gold] = right_count_list[gold] + 1

        # count for the predicted items
        for label in sample_predict_id_list:
            predicted_count_list[label] = predicted_count_list[label] + 1
    ...
    for i, label in id2label.items():
        precision_dict[label], recall_dict[label], fscore_dict[label] = _precision_recall_f1(right_count_list[i],
                                                                                             ...
                                                                                             gold_count_list[i])
        right_total = right_total + right_count_list[i]
        gold_total = gold_total + gold_count_list[i]
        predict_total = predict_total + predicted_count_list[i]
...
def evaluate_rcv_layer(epoch_predicts, epoch_labels, vocab, threshold=0.5, top_k=None):
    ...
    for i, label in id2label.items():
        ...
        if label in rcv1_layer1:
            right_total_layer1 = right_total_layer1 + right_count_list[i]
            gold_total_layer1 = gold_total_layer1 + gold_count_list[i]
            predict_total_layer1 = predict_total_layer1 + predicted_count_list[i]
        elif label in rcv1_layer2:
            right_total_layer2 = right_total_layer2 + right_count_list[i]
            gold_total_layer2 = gold_total_layer2 + gold_count_list[i]
            predict_total_layer2 = predict_total_layer2 + predicted_count_list[i]
        else:
            right_total_layer3 = right_total_layer3 + right_count_list[i]
            gold_total_layer3 = gold_total_layer3 + gold_count_list[i]
            predict_total_layer3 = predict_total_layer3 + predicted_count_list[i]
...
def evaluate_math_layer(epoch_predicts, epoch_labels, vocab, threshold=0.5, top_k=None):
    ...
    for i, label in id2label.items():
        ...
        if label in rcv1_layer1:
            ...
            right_total_layer4 = right_total_layer4 + right_count_list[i]
            gold_total_layer4 = gold_total_layer4 + gold_count_list[i]
            predict_total_layer4 = predict_total_layer4 + predicted_count_list[i]
...
def evaluate_math_tail(epoch_predicts, epoch_labels, vocab, threshold=0.5, top_k=None):
    ...
    for sample_predict, sample_gold in zip(epoch_predicts, epoch_gold):
        ...
        for gold in sample_gold:
            ...
            for label in sample_predict_id_list:
                if gold == label:
                    right_count_list[gold] += right_count_list[gold]
