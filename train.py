...
from train_modules.criterions import ClassificationLoss, MarginRankingLoss, ResampleLoss
...
def train(config):
    ...
    # build up model
    dlac = DLAC(config, corpus_vocab, model_mode='TRAIN')
    dlac.to(config.train.device_setting.device)
    ...
    #                          logit_reg=dict(),
    #                          map_param=dict(alpha=1.0, beta=10.0, gamma=0.9),
    #                          class_freq=corpus_vocab.freqs['label'], train_num=30070)

    # define ranking loss
    criterion_ranking = MarginRankingLoss(config)
    optimize = set_optimizer(config, dlac)

    # get epoch trainer
    trainer = Trainer(model=dlac,
    ...
    for epoch in range(config.train.start_epoch, config.train.end_epoch):
        ...
        if os.path.isfile(best_epoch_model_file):
            load_checkpoint(best_epoch_model_file, model=dlac,
                            config=config,
                            optimizer=optimize)
            performance = trainer.eval(test_loader, best_epoch_dev[0])
            if performance['micro_f1'] > best_performance_test[0]:
                logger.info(
                    'TEST Improve Micro-F1 {}% --> {}%'.format(best_performance_test[0], performance['micro_f1']))
