...
from models.tail_network import TailNet
import torch.nn.functional as F

class DLACTP(nn.Module):
    def __init__(self, config, label_map, graph_model, device, model_mode, graph_model_label=None):
        ...
        self.tail_model = TailNet(config, self.graph_model_label, label_map)
    ...
    def forward(self, inputs):
        ...
        """
        if inputs[1] == "TRAIN":
            text_feature, mode, head_label_list, label_repre = inputs
        else:
        ...
        if inputs[1] == "TRAIN":
            tail_logit = self.tail_model(label_wise_text_feature.view(label_wise_text_feature.shape[0], -1), head_label_list, label_repre)
            return logits, tail_logit
        else:
            return logits
