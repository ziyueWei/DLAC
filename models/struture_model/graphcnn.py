...
class HierarchyGCNModule(nn.Module):
    def __init__(self,
        ...
        #  bottom-up child sum
        in_prob = in_adj
        self.adj_matrix = Parameter(torch.Tensor(in_prob))  # self.adj_matrix: [141, 141]
        self.edge_bias = Parameter(torch.Tensor(num_nodes, in_dim))  # self.edge_bias: [141, 300]
        self.gate_weight = Parameter(torch.Tensor(in_dim, 1))   # self.gate_weight: [300, 1]
        self.bias_gate = Parameter(torch.Tensor(num_nodes, 1))   # self.bias_gate: [141, 1]
        self.activation = nn.ReLU()
        self.origin_adj = torch.Tensor(np.where(in_adj <= 0, in_adj, 1.0)).to(device)  # self.origin_adj: [141, 141]
        # top-down: parent to child
        self.out_adj_matrix = Parameter(torch.Tensor(out_adj))  # self.out_adj_matrix: [141, 141]
        self.out_edge_bias = Parameter(torch.Tensor(num_nodes, in_dim))  # self.out_edge_bias: [141, 300]
        self.out_gate_weight = Parameter(torch.Tensor(in_dim, 1))  # self.out_gate_weight: [300, 1]
        self.out_bias_gate = Parameter(torch.Tensor(num_nodes, 1))  # self.out_bias_gate: [141, 1]
        self.loop_gate = Parameter(torch.Tensor(in_dim, 1))  # self.loop_gate: [300, 1]
    ...
    def forward(self, inputs):
        ...
        """
        h_ = inputs  # h_:[64, 141, 300]
        message_ = torch.zeros_like(h_).to(self.device)  # message:[64, 141, 300]

        # h_in:[64, 141, 300]  self.origin:[141, 141]  self.adj_matrix:[141, 141]
        h_in_ = torch.matmul(self.origin_adj * self.adj_matrix, h_)
        in_ = h_in_ + self.edge_bias  # self.edge_bias: [141, 300]  in:[64, 141, 300]
        in_ = in_
        # N,1,dim
        in_gate_ = torch.matmul(h_, self.gate_weight)  # self.gate_weight:[300, 1] in_gate_:[64, 141, 1]
        # N, 1
        in_gate_ = in_gate_ + self.bias_gate  # self.bias_gate: [141, 1] in_gate_:[64, 141, 1]
        in_ = in_ * F.sigmoid(in_gate_)  # in_:[64, 141, 300]
        in_ = self.dropout(in_)
        message_ = message_ + in_  # batch, N, in_dim
        ...
        message_ = message_ + out_
        
        loop_gate = torch.matmul(h_, self.loop_gate)  # self.loop_gate:[300, 1]
        loop_ = h_ * F.sigmoid(loop_gate)
        loop_ = self.dropout(loop_)
        message_ = message_ + loop_
